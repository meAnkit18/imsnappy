...
- Cormorant Garamond appears in every primary page title, welcome state, and major empty-state heading; Inter remains for controls, body text, labels, and annotations.
- Archive Mint (#A7E5D3) and companion pastels are atmospheric only: they may not become hard status fills, toggle fills, or navigational hierarchy.
- Empty states read as editorial notes from a capable agent: one specific observation, one plain system-state explanation, and one direct next action.
- The home workspace opens as a considered editorial welcome with a restrained wordmark, asymmetric margin-note context, and a short set of specific work prompts before the first message arrives.
